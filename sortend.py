...
def sortend(lista):
    ...
    # melléknevek           oki
    advre = re.compile(r"((enn|ennu|hv(em|or)|her|der|hvorfor|hva|hvordan|hvorledes|åssen|inatt|imorgen|idag|ikveld|ikke|mer|mest|fler|flere|flest|få|mere|mange|mye|meget|ofte|ofte(re|st|ste)|aldri|alltid|sjeldsynt|kan(skje|hende)|selve|sjølve|selveste|sakte)|[a-zåøæ]*(igens|igvis)$)")
    # határozószók          oki
    # ska kollas innan adjektiven
    pronomre = re.compile(r"((de[ntm]|noen|noe|inge[nt]*|[smd]eg|oss|dere|deres|de|vi|[smd]i(n|tt|ne)|vår|vår[te]|hans|hennes|jeg|du|ha[nm]|hun|dens|dets|denne|dette|disse|dennes|dettes|disses|selv|sjøl|hvilke[nt]|hvilke)$)")
    # névmások              oki
